...
- **Need to make a decision** between options — use [Evaluate Options](../evaluate-options/SKILL.md)
- **Auditing against a checklist** — use [Checklist Runner](../checklist-runner/SKILL.md)
- **Comparing architecture** against codex recommendations — use [Architecture Review](../architecture-review/SKILL.md)
- **Quick lookup** of a single best practice — just read the facet file directly
...
- **Phase 1**: Confirm the facet/experience exists by checking for its `README.md`. If not found, list available facets.
- **Phase 2**: For each perspective file read, confirm it has real content (not just template placeholders). Skip templated files and note them.
- **Phase 3**: Confirm cross-references are real facets that exist in the codex — don't link to non-existent facets.

## Worked Example

**Input:** `Deep dive into authentication for the finance-mfe project`

**Key steps:**
1. Loaded `facets/authentication/README.md` — 5 perspectives available
2. User chose "Compare against my codebase"
3. Read `best-practices.md` and `architecture.md`, then explored `finance-mfe/src/`
4. Found: auth guard in `router/index.ts` using `@pax8/auth` library, token refresh in `composables/useAuth.ts`
5. Gap: no silent token renewal — user gets logged out when token expires during a session

**Output excerpt:**
```markdown
### Authentication — Codebase Comparison

**Current State:** Auth guard on routes using `@pax8/auth`. Token stored in memory. No silent renewal.
**Codex Recommendation:** Implement silent token renewal to avoid session interruption.
**Alignment:** Partially Aligned

**Gaps:**
- No silent token renewal — users are logged out when access token expires
- No auth error boundary component for graceful 401 handling

**Recommended Actions:**
- [ ] Add token refresh interceptor in `useAuth.ts`
- [ ] Create `AuthErrorBoundary.vue` component
```
