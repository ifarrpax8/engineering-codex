...
> **Stack Callout — Pax8**: Pax8 provides the `rest-query` Spring Boot starter library (RFC-0039) that implements Lucene-based querying with faceted search, field boosting, fuzzy matching, and business rule filtering out of the box. Use `@QueryField` annotations and `ElasticsearchQueryRepository` for standardised query endpoints. See the [REST Query API Development Guide](https://pax8.atlassian.net/wiki/spaces/DD/pages/2606694731) for implementation details.
...
The [W3C Trace Context](https://www.w3.org/TR/trace-context/) standard defines how trace context is propagated across service boundaries. Two headers carry the context:
...
- **`tracestate`** (optional): Vendor-specific trace data as key-value pairs
  - Example: `vendor1=value1,vendor2=value2`

**How it works**:
1. The first service in a request chain generates a `traceparent` with a new trace ID
2. Each downstream service extracts the trace context, creates a child span, and propagates updated headers to further downstream calls
3. The trace ID stays the same across the entire distributed transaction; the parent ID changes at each hop

**Benefits**:
- End-to-end tracing across microservices without vendor lock-in
- Compatible with OpenTelemetry and all major observability platforms
- Enables request correlation in logs, metrics, and traces

**Implementation**: Most frameworks handle propagation automatically when instrumented with OpenTelemetry. See [Observability](../observability/architecture.md) for instrumentation details.

### Trace Context vs Idempotency Key

These serve different purposes and should not be confused:

| Concern | Trace Context (`traceparent`) | Idempotency Key |
|---------|------------------------------|-----------------|
| **Purpose** | Trace a request through distributed systems | Prevent duplicate operations on retry |
| **Generated by** | First service in chain (or client) | Client, before the request |
| **Uniqueness** | Unique per distributed trace | Reused when retrying the same operation |
| **Propagation** | Forwarded through entire request chain | Consumed by the receiving service only |
| **Storage** | In-memory spans, exported to tracing backend | Persistent storage with response cache |

On a retry, the client sends the **same** idempotency key but the tracing system generates a **new** span (same trace ID, new parent ID).
